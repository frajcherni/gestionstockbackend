...
exports.createTransfer = async (req, res) => {
  ...
  try {
    ...
    for (const itemData of articles) {
      ...
      const availableStock = sourceStock ? parseInt(sourceStock.qte || 0) : 0;
      ...
      if (destStock) {
        destStock.qte = parseInt(destStock.qte || 0) + parseInt(qte);
      } else {
    ...
    for (const itemData of articles) {
      ...
      const totalArticleStock = articleStocks.reduce((sum, stock) => sum + parseInt(stock.qte || 0), 0);
...
exports.updateTransfer = async (req, res) => {
  ...
  try {
    ...
    for (const oldItem of transfer.items) {
      ...
      if (sourceStock) {
        sourceStock.qte = parseInt(sourceStock.qte || 0) + parseInt(oldItem.qte || 0);
        await stockRepo.save(sourceStock);
      }
      ...
      if (destStock) {
        destStock.qte = parseInt(destStock.qte || 0) - parseInt(oldItem.qte || 0);
        if (destStock.qte <= 0) {
  ...
  try {
    ...
    if (sourceDepot && destinationDepot) {
      ...
      for (const item of transfer.items) {
        ...
        if (sourceStock) {
          sourceStock.qte = parseInt(sourceStock.qte || 0) + parseInt(item.qte || 0);
          await stockRepo.save(sourceStock);
        }
        ...
        if (destStock) {
          destStock.qte = parseInt(destStock.qte || 0) - parseInt(item.qte || 0);
          if (destStock.qte <= 0) {
            await stockRepo.delete(destStock.id);
          } else {
            await stockRepo.save(destStock);
